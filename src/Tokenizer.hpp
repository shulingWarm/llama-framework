...
{
	//单词的索引表
	std::unordered_map<std::string,TokenId> tokenMap;
	//从token到单词的索引表
	std::unordered_map<TokenId,std::string> tokenStrMap;
public:
	...
	void loadVocabulary(std::fstream& fileHandle)
	{
		...
		for(auto idPair=0;idPair<pairNum;++idPair)
		{
			...
			//读取对应的id
			auto tokenId = StreamReader::read<TokenId>(fileHandle);
			//在map里面记录这个pair
			tokenMap[str] = tokenId;
			tokenStrMap[tokenId] = str;
		}
	}

	//从单词到token的转换
	TokenId word2Token(const std::string& word)
	{
		//判断map里面是否存在
	...
	//把字符串转换成token列表
	void str2Token(const std::string& str,std::vector<TokenId>& tokenList)
	{
		//目前已经处理到的id位置
		auto currId = 0U;
		//临时的字符串buffer
	...
	//把token列表转换成字符串
	std::string token2Str(const TokenId* tokenList,int tokenNum)
	{
		std::stringstream sstream;
		//遍历每个token
		for(int i=0;i<tokenNum;++i){
			MY_ASSERT(tokenStrMap.count(tokenList[i]));
			sstream << tokenStrMap[tokenList[i]];
		}
		return sstream.str();
	}
};