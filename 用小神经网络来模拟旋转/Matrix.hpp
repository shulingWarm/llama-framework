...
//这个里面还是一般意义上的矩阵，但现在不再使用它了
//不过到时候还需要另一种矩阵，那个矩阵的每一列表示的是一个单位向量，但不同列的单位向量可能不是正交的。
...
Matrix loadMatrix(std::fstream& fileHandle)
{
	...
	//初始化矩阵里的两个cuda数据
	mat.scaleList = (float*)initFromCpuData((char*)scaleData.data(),sizeof(float)*mat.rowNum);
	mat.data = (FeatType*)initFromCpuData((char*)numData.data(),sizeof(FeatType)*dataNum);
	//传值的形式，返回已经初始化过的矩阵
	return mat;
}
...
{
	return mat->data + idRow*mat->colNum;
}